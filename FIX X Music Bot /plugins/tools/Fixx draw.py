...
@app.on_message(filters.command(["vashudraw", "vashucreate", "vashuimagine", "vashudream"]))
async def vashudraw(_: app, m: t.Message):
    global Database
    prompt = getText(m)
    if prompt is None:
...
@app.on_callback_query(filters.regex(pattern=r"^vd.(.*)"))
