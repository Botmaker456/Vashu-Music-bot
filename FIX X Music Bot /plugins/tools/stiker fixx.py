...
@app.on_message(filters.command("vst"))
...
@app.on_message(filters.command("vpackkang"))
async def _packkang(app: app, message):
    txt = await message.reply_text("**ᴘʀᴏᴄᴇssɪɴɢ....**")
    if not message.reply_to_message:
        await txt.edit('ʀᴇᴘʟʏ ᴛᴏ ᴍᴇssᴀɢᴇ / 𝙈𝙀𝙎𝙎𝘼𝙂𝙀 𝙆𝘼 𝙍𝙀𝙋𝙇𝙔 𝙏𝙊 𝘿𝙊 𝙔𝙍𝙍')
        return
    if not message.reply_to_message.sticker:
        await txt.edit('ʀᴇᴘʟʏ ᴛᴏ sᴛɪᴄᴋᴇʀ / 𝙎𝙏𝙄𝙆𝙀𝙍 𝙆𝘼 𝙍𝙀𝙋𝙇𝙔 𝙏𝙊 𝘿𝙊 𝙔𝙍𝙍')
        return
    if message.reply_to_message.sticker.is_animated or message.reply_to_message.sticker.is_video:
        return await txt.edit(
            "ʀᴇᴘʟʏ ᴛᴏ ᴀ ɴᴏɴ-ᴀɴɪᴍᴀᴛᴇᴅ sᴛɪᴄᴋᴇʀ / 𝙀𝙆 𝙉𝙊𝙉 𝘼𝙉𝙄𝙈𝘼𝙏𝙀𝘿 𝙎𝙏𝙄𝙆𝙀𝙍 𝙆𝘼 𝙍𝙀𝙋𝙇𝙔 𝙏𝙊 𝙅𝘼𝘽𝙃𝙄 𝘽𝘼𝙉𝘼 𝙋𝘼𝙔𝙀𝙂𝘼")
    if len(message.command) < 2:
...
###### sticker id =
@app.on_message(filters.command(["vstickerid", "vstid"]))
