...
@app.on_message(
    filters.command(["vadmincache", "vreload", "vrefresh", "vashureload"],
                    prefixes=["/", "!", "%", ",", "", ".", "@", "#"]) & filters.group & ~BANNED_USERS
)
@language
async def vreload_admin_cache(client, message: Message, _):
    try:
...
@app.on_message(filters.command(["vreboot", "vashureboot"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def restartbot(client, message: Message, _):
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await FIXX.stop_stream_force(chat_id)
        except:
            pass
...
@app.on_message(
    filters.command("vdone", "vashudone")
    & filters.private
            ...
            [
                [
                    InlineKeyboardButton(
                        "• нαϲкє𝚍 ву  •", url=f"https://t.me/iam_fixx")
                ]
            ]
        ),
    )
...
@app.on_callback_query(filters.regex("vclose", "vashuclose") & ~BANNED_USERS)
async def vclose_menu(_, query: CallbackQuery):
    try:
        await query.answer()
...
@app.on_callback_query(filters.regex("vstop_downloading") & ~BANNED_USERS)
@ActualAdminCB
async def vstop_download(client, CallbackQuery: CallbackQuery, _):
