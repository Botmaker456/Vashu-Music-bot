...
from FIXXMUSIC.utils import DAXXBin, get_channeplayCB, seconds_to_min
from FIXXMUSIC.utils.database import get_cmode, is_active_chat, is_music_playing
from FIXXMUSIC.utils.decorators.language import language, languageCB
from FIXXMUSIC.utils.inline import queue_back_markup, queue_markup
...
@app.on_message(
    filters.command(["Vqueue", "cqueue", "Vplayer", "cplayer", "playing", "cplaying"])
    & filters.group
    & ~BANNED_USERS
)
@language
...
@app.on_callback_query(filters.regex("VGetTimer") & ~BANNED_USERS)
async def vquite_timer(client, CallbackQuery: CallbackQuery):
    try:
...
@app.on_callback_query(filters.regex("VGetQueued") & ~BANNED_USERS)
@languageCB
async def vqueued_tracks(client, CallbackQuery: CallbackQuery, _):
...
@app.on_callback_query(filters.regex("vqueue_back_timer") & ~BANNED_USERS)
@languageCB
async def vqueue_back(client, CallbackQuery: CallbackQuery, _):
    callback_data = CallbackQuery.data.strip()
