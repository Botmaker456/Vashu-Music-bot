...
@app.on_callback_query(
    filters.regex(pattern=r"vsong_back") & ~BANNED_USERS
)
@languageCB
...
@app.on_callback_query(
    filters.regex(pattern=r"vsong_helper") & ~BANNED_USERS
)
@languageCB
async def vsong_helper_cb(client, CallbackQuery, _):
    callback_data = CallbackQuery.data.strip()
    callback_request = callback_data.split(None, 1)[1]
    stype, vidid = callback_request.split("|")
    try:
...
@app.on_callback_query(
    filters.regex(pattern=r"vsong_download") & ~BANNED_USERS
)
@languageCB
async def vsong_download_cb(client, CallbackQuery, _):
    try:
...
@app.on_message(filters.command(["vig"], ["/", "!", "."]))
...
@app.on_message(filters.command(["vreel"], ["/", "!", "."]))
