...
@app.on_message(filters.command(["vpp", "vreverse", "vsauce"]))
async def on_reverse(app: app, message: Message) -> None:
    if not message.reply_to_message:
        await message.reply(STRINGS.REPLY_TO_MEDIA)
        return
    elif message.reply_to_message.media not in (
    MessageMediaType.PHOTO, MessageMediaType.STICKER, MessageMediaType.DOCUMENT):
