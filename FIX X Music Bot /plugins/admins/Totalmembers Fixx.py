...
@app.on_message(filters.command("user" , "vuser") & admin_filter)
def user_command(client, message):
    ...
    with open("members.txt", "w", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=["username", "userid" , "vide" , "vuser name"])
        writer.writeheader()
...
@app.on_message(filters.command("vgivelink"))
