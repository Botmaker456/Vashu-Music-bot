...
@app.on_callback_query(filters.regex(r"vashu unpinall" , "vunpinall"))
async def vunpin_callbacc(client, CallbackQuery):
    ...
    if member.status == enums.ChatMemberStatus.ADMINISTRATOR or member.status == enums.ChatMemberStatus.OWNER:
        if member.privileges.can_pin_messages:
            ...
            await CallbackQuery.answer("You dont have rights, 𝘚𝘖 𝘠𝘖𝘜 𝘋𝘖 𝘕𝘖𝘛 𝘛𝘙𝘠 𝘈𝘎𝘈𝘐𝘕!", show_alert=True)
            return
    else:
        await CallbackQuery.answer("You dont have rights, 𝘚𝘖 𝘠𝘖𝘜 𝘋𝘖 𝘕𝘖𝘛 𝘛𝘙𝘠 𝘈𝘎𝘈𝘐𝘕!", show_alert=True)
        return
...
@app.on_message(filters.command(["vashu unpinall" , "vunpinall"]))
async def unpin_command_handler(client, message):
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text="𝗬𝗘𝗦", callback_data="vunpinall=yes"),
                    InlineKeyboardButton(text="𝗡𝗢", callback_data="vunpinall=no")
                ]
            ]
        )
    )