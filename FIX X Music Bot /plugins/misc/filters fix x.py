...
from FIXXMUSIC.mongo.filtersdb import *
from FIXXMUSIC.utils.filters_func import GetFIlterMessage, get_text_reason, SendFilterMessage
from FIXXMUSIC.utils.yumidb import user_admin
from pyrogram import filters
from pyrogram.enums import ChatMemberStatus
from pyrogram.types import CallbackQuery, InlineKeyboardButton, InlineKeyboardMarkup


@app.on_message(filters.command("vashu_filter" , "vfilter") & admin_filter)
@user_admin
async def _filter(client, message):
    ...
    if (
        ...
        await message.reply("You need to give the filter a name!/𝘼𝙍𝙀𝙀 𝙎𝙊𝙉𝙊 𝙈𝙐𝙅𝙃𝙀 𝙆𝙊𝙄 𝙁𝙄𝙇𝙏𝙀𝙍 𝙉𝘼𝙈𝙀 𝙏𝙊 𝘿𝙊 𝙅𝘼𝘽𝙃𝙄 𝙇𝘼𝙂𝘼 𝙋𝘼𝙐𝙉𝙂𝙄")
        return
    ...
    if (
        ...
        await message.reply("You need to give the filter some content!/𝘼𝙍𝙀𝙀 𝙎𝙊𝙉𝙊 𝙈𝙐𝙅𝙃𝙀 𝙆𝙊𝙄 𝙁𝙄𝙇𝙏𝙀𝙍 𝙉𝘼𝙈𝙀 𝙏𝙊 𝘿𝙊 𝙅𝘼𝘽𝙃𝙄 𝙇𝘼𝙂𝘼 𝙋𝘼𝙐𝙉𝙂𝙄")
        return
    ...
    await message.reply(
        f"vashuSaved filter '`{filter_name}`'."
    )
...
@app.on_message(filters.command('vashufilters' , 'vfilters') & filters.group)
...
@app.on_message(filters.command('vashustopall' , 'vstopall') & admin_filter)
...
@app.on_callback_query(filters.regex("^vcustfilters_"))
...
@app.on_message(filters.command('vashustopfilter') & admin_filter)
@user_admin
async def vstop(client, message):
    chat_id = message.chat.id
