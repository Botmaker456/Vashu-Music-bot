...
from FIXXMUSIC.misc import HAPP, SUDOERS, XCB
...
from FIXXMUSIC.utils.pastebin import FIXXBin
...
@app.on_message(filters.command(["vgetlog", "vlogs", "vgetlogs"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]) & SUDOERS)
@language
...
@app.on_message(filters.command(["vupdate", "vgitpull"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]) & SUDOERS)
@language
...
async def restart_(_, message):
    ...
    for x in ac_chats:
        try:
            await app.send_message(
                chat_id=int(x),
                text=f"{app.mention} ɪs ʀᴇsᴛᴀʀᴛɪɴɢ...\n\nʏᴏᴜ ᴄᴀɴ sᴛᴀʀᴛ ᴩʟᴀʏɪɴɢ ᴀɢᴀɪɴ ᴀғᴛᴇʀ 15-20 sᴇᴄᴏɴᴅs/ 𝙎𝙐𝙉𝙊 𝙅𝙄 𝘼𝘽 5 𝙈𝙄𝙉𝙐𝙏𝙀 𝙍𝙐𝙆 𝙅𝘼𝙊 𝙈𝘼𝙔 𝘽𝙊𝙏 𝙆𝙊 𝙍𝙀𝙎𝙏𝘼𝙍𝙏 𝙆𝘼𝙍 𝙍𝘼𝙃𝙄 𝙃𝙐 𝘼𝙋 𝙆𝙄 𝘾𝙊𝙈𝙈𝘼𝙉𝘿 𝙎𝙀.",
            )
            await remove_active_chat(x)
            await remove_active_video_chat(x)
        except:
