...
from FIXXMUSIC.utils.database import blacklist_chat, blacklisted_chats, whitelist_chat
from FIXXMUSIC.utils.decorators.language import language
from config import BANNED_USERS


@app.on_message(filters.command(["vblchat", "vashublacklistchat" , "vashu_blacklist"]) & SUDOERS)
@language
async def vblacklist_chat_func(client, message: Message, _):
...
@app.on_message(
    filters.command(["vwhitelistchat", "vunblacklistchat", "vunblchat"]) & SUDOERS
)
@language
async def vwhite_funciton(client, message: Message, _):
    if len(message.command) != 2:
...
@app.on_message(filters.command(["vblchats", "vblacklistedchats"]) & ~BANNED_USERS)
@language
