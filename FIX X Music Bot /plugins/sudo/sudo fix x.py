...
from FIXXMUSIC.utils.database import add_sudo, remove_sudo
from FIXXMUSIC.utils.decorators.language import language
from FIXXMUSIC.utils.extraction import extract_user
from FIXXMUSIC.utils.inline import close_markup
from config import BANNED_USERS, OWNER_ID


@app.on_message(
    filters.command(["vaddsudo"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]) & filters.user(OWNER_ID))
@language
...
@app.on_message(
    filters.command(["vdelsudo", "vrmsudo"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]) & filters.user(OWNER_ID))
@language
...
@app.on_message(filters.command(["sudolist", "vlistsudo", "vsudoers"],
                                prefixes=["/", "!", "%", ",", "", ".", "@", "#"]) & ~BANNED_USERS)
async def sudoers_list(client, message: Message):
    keyboard = [[InlineKeyboardButton("๏ ᴠɪᴇᴡ sᴜᴅᴏʟɪsᴛ ๏", callback_data="check_sudo_list")]]
    reply_markups = InlineKeyboardMarkup(keyboard)

    # await message.reply_photo(photo="https://telegra.ph/file/01f48282df85ff08672c0.jpg", caption="**» ᴄʜᴇᴄᴋ sᴜᴅᴏ ʟɪsᴛ ʙʏ ɢɪᴠᴇɴ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ.**\n\n**» ɴᴏᴛᴇ:**  ᴏɴʟʏ sᴜᴅᴏ ᴜsᴇʀs ᴄᴀɴ ᴠɪᴇᴡ. ", reply_markup=reply_markups)
    await message.reply_video(video="https://telegra.ph/file/01f48282df85ff08672c0.jpg",
...
async def check_sudo_list(client, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id not in SUDOERS:
        return await callback_query.answer(
            "𝐍𝐢𝐤𝐚𝐥 𝐑𝐚𝐧𝐝𝐢 V𝐚𝐥𝐚 𝐒𝐮𝐝𝐨𝐥𝐢𝐬𝐭 𝐃𝐞𝐤𝐡𝐧𝐞 𝐀𝐚𝐲𝐚 𝐇𝐚𝐢 𝐛𝐚𝐝𝐚🖕😎😂",
            show_alert=True)
    else:
