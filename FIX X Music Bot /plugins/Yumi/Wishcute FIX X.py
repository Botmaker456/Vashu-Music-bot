...
async def wish(_, m):
    if len(m.command) < 2:
        await m.reply("ᴀᴅᴅ ᴡɪꜱʜ ʙᴀʙʏ👻 𝕀 ℙℝ𝔸𝕐 𝕋𝕆 𝔾𝕆𝔻 𝕂𝕀 𝔸ℙ 𝕂𝕀 𝕎𝕀𝕊ℍ ℙ𝕌ℝ𝔼 ℍ𝕆𝕐𝔼 🙏 🕉️!")
        return

    api = requests.get("https://nekos.best/api/v2/happy").json()
    url = api["results"][0]['url']
    text = m.text.split(None, 1)[1]
    wish_count = random.randint(1, 110)
...
async def cute(_, message):
    ...
    mm = random.randint(1, 110)
    CUTE = f"🍑 {mention} {mm}% ℂ𝕌𝕋𝔼 𝔹𝔸𝔹𝕐 ℍ𝕆 𝕋𝕌𝕄 𝕄𝔼ℝ𝔼😏"
