...
from FIXXMUSIC.plugins.play import play
from FIXXMUSIC.plugins.play.pornplay import play
...
@app.on_callback_query(filters.regex("^pornplay"))
async def pornplay_callback(_, query):
...
@app.on_message(filters.command("xxx"))
async def get_random_video_info(client, message):
    if len(message.command) == 1:
        await message.reply("Please provide a title to search.")
        return
